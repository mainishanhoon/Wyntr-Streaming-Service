...
def Admin_Interface(self):
    ...
    scrollable_description_frame = CTkScrollableFrame(master=details_frame, fg_color='#E5AA70', scrollbar_button_color='#834333',
                                          scrollbar_button_hover_color='#954535', width=340, height=180)
    scrollable_description_frame.pack(fill='both', expand=True, padx=2, pady=2)

    CTkLabel(master=scrollable_description_frame, text='Description', text_color='#954535',
             font=('Stack Sans Text', 13, 'bold')).place(
        x=15, y=265)
    Description = CTkTextbox(master=scrollable_description_frame, border_width=2, fg_color='#F3C892',
    ...
    CTkButton(master=filter_frame, text='SHOW ALL', command=lambda: ShowData(), font=('Dela Gothic One', 12),
              fg_color='#954535', hover_color='#7B3F00', cursor='hand2', hover=True, height=30, width=100).place(
        x=790, y=10)
    ...
    Media_Table.pack(fill='both', expand=False)

    column_widths = [300, 50, 50, 50, 50, 50]

    for col, w in enumerate(column_widths):
        Media_Table.edit_column(col, width=w)

    Media_Table.bind('<ButtonRelease-1>', GetData)

    Media_Table.bind('<Return>', lambda: SearchData(self))